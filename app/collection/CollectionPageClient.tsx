...
export default function Dashboard() {
  ...
  const [songSearchIndex, setSongSearchIndex] = useState<
  ...
  const refreshSelectedCustomAlbum = async (albumId: number) => {
    ...
    try {
      ...
      const normalizedAlbum = {
        ...
        detailsRef:
          typeof data.detailsRef === "string" ? data.detailsRef : null,
    ...
    const loadSongIndex = async () => {
      const entries = await Promise.all(
        missingAlbums.map(async (album) => {
          try {
            if (album.detailsRef) {
              const sharedDetails = await getSharedAlbumDetails(
                album.detailsRef,
              );
              return [
                album.id,
            ...
            return [
              album.id,
              extractTracklistSearchText(rawTracklist),
            ] as const;
          } catch {
    ...
    <div className="collection-container flex flex-col min-h-full gap-4">
      ...
      <div className="flex gap-4 h-10 items-stretch">
        ...
        {sortBy === "releaseDate" && (
          ...
          >
            {releaseDateAsc ? <ArrowUp size={15} /> : <ArrowDown size={15} />}
          </button>
        )}
      </div>
