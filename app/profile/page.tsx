...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-full flex flex-col gap-4">
      <h1 className="text-4xl sm:text-5xl mb-4">{t(locale, "profile")}</h1>
      <h2 className="text-md sm:text-xl font-semibold">
        {t(locale, "helloName", `${profile.firstName} ${profile.lastName}`)}
      </h2>
      <div className="flex flex-col gap-2 text-base">
        <p>
          {t(locale, "firstName")}: {profile.firstName}
        </p>
        <p>
          {t(locale, "lastName")}: {profile.lastName || "-"}
        </p>
        <p>
          {t(locale, "email")}: {profile.email || "-"}
        </p>
      </div>
    </div>
  );
